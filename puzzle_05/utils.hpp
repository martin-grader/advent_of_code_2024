...
std::vector<std::tuple<int, int>> get_relevant_order_rules(const std::vector<int> &unordered_numbers,
    ...
    for (auto [first, second] : order_rules) {
        const bool first_found =
            std::find(unordered_numbers.begin(), unordered_numbers.end(), first) != unordered_numbers.end();
        const bool second_found =
            std::find(unordered_numbers.begin(), unordered_numbers.end(), second) != unordered_numbers.end();
